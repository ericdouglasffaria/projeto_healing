...
def consulta_area_medico(request, id_consulta):
    if not is_medico(request.user):
        messages.add_message(request, constants.WARNING, 'Somente médicos podem acessar essa área.')
        return redirect('/usuarios/sair')
...
def finalizar_consulta(request, id_consulta):
    ...
    if request.user != consulta.data_aberta.user:
        messages.add_message(request, constants.ERROR, 'Dr(a), você não pode Finalizar essa Consulta.')
        return redirect(f'/medicos/consulta_area_medico/')
...
def add_documento(request, id_consulta):
    ...
    if consulta.data_aberta.user != request.user:
        messages.add_message(request, constants.ERROR, 'Dr(a), não pode anexar Documento nessa Consulta.')
        return redirect(f'/medicos/consulta_area_medico/{id_consulta}')
