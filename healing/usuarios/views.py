...
def cadastro(request):
    ...
    if request.method == 'GET':
     return render(request, 'cadastro.html')
        ...
        if len(senha) < 6:
            messages.add_message(request, constants.ERROR, 'A senha deve possuir pelo mais de 6 caracteres')
            return redirect('/usuarios/cadastro')
...
def login_view(request):
    if request.method == 'GET':
        ...
        messages.add_message(request, constants.ERROR, 'Usuário ou Senha Invalida.')
        return redirect('/usuarios/login')
